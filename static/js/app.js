...
var App = {
  ...
  async playSongItem(song, source, skipQueueAdd) {
    ...
    if (needsSpeakerProxy) {
      try {
        var resolvedSong = fallbackResp.data.song || {};
    ...
    if (resp.success || resp.raw !== undefined) {
      ...
      // 时长来源: song.interval("03:58") → song.duration → 240默认值
      var dur = 0;
      if (typeof song.interval === 'string') {
        dur = this.parseDuration(song.interval);
      } else {
        dur = parseInt(song.duration) || parseInt(song.interval) || 0;
      }
      ...
      if (lists[i].id === id) {
        ...
        if (songs.length > 0) {
          ...
          this.playQueue = songs.map(function(s) {
            return { id: s.id, name: s.name, singer: s.singer, source: s.source, cover: s.meta?.picUrl, _raw: s.meta };
          });
